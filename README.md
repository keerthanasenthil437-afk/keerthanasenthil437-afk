<p align="center">
  <img src="ChatGPT Image May 2, 2026, 11_10_18 AM.png" width="100%">
</p>

# 👋 Hi, I'm Keerthana

🎓 2nd Year Computer Science Engineering Student (Madurai, Tamil Nadu)  
💻 Aspiring Software Developer | Problem Solver  
🚀 Interested in Software Development, Java, and Web Technologies  

---

## 🔧 Tech Stack
- C Programming  
- Java (OOP + GUI Basics)  
- HTML, CSS  
- Data Structures & Algorithms (Learning)

---

## 💼 Projects

🐍 Snake Game (C) – Console game with scoring system  
❌ Tic Tac Toe (C) – Two player game  
🛒 Shopping Cart (Java GUI) – Basic e-commerce simulation  
🧠 Quiz Application (Java GUI) – Interactive quiz system  
🌐 Portfolio Website – Personal website using HTML/CSS  

---

## 📌 Goals
- Improve DSA skills  
- Build full-stack projects  
- Contribute to open source  

---

## 📫 Contact
📧 Email: keerthanasenthil437@gmail.com  
🔗 LinkedIn: https://linkedin.com/in/keerthanasenthilkumar6  
💻 GitHub: https://github.com/keerthanasenthil437-afk  
